...
template<>
SMatrix<double>::SMatrix(const SMatrix<double> &inMat){
    this->initialize_from_mat(inMat, DEFAULT_TOL);
}
...
template<>
SMatrix<double>::SMatrix(const FMatrix<double> &inMat){
    this->initialize_from_mat(inMat, DEFAULT_TOL);
}
...
bool SMatrix<double>::isZero(double element) const {
    return this->isZero(element, DEFAULT_TOL);
}
...
int SMatrix<T>::retrieveElement(int row, int col, T *element) const {
    ...
    *element = 0;
    ...
    for (int i = 0; i < rowSize; i++) {
        if (this->colInd[colOffset + i] == col) {
...
int SMatrix<T>::getRow(int row, T *res) const {
    ...
    for (; col < this->colLen; col++){
        if ( colInd[colOffset + colMatch] == col){
...
template <class T>
int SMatrix<T>::rowScale(int i, T a){
    for (int index = this->rowPtr[i]; index < this->rowPtr[i+1]; index++) {
        this->value[index] = a*this->value[index];
    }
    return M_SUCCESS;
}
